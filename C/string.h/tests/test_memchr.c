...
START_TEST(find_symbol_in_start) {
  s21_size_t n_byte = 6;
  char str[10] = "Atring";
  int find_byte = 'A';
...
START_TEST(find_float_in_array) {
  float array[] = {1, 912, 234, 666, 50, 99, 100};
...
Suite *suite_memchr(void) {
  ...
  tcase_add_test(tc, find_end_str);
  tcase_add_test(tc, find_symbol_in_start);
  tcase_add_test(tc, find_symbol_in_end);
  tcase_add_test(tc, find_num_in_mid);
  tcase_add_test(tc, find_num_in_array);
  tcase_add_test(tc, find_float_in_array);
  tcase_add_test(tc, easy_case_string);
